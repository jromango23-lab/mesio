...
export default function PublicMenu() {
  ...
  if (restaurant && !restaurant.is_active) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4 text-center">
        <div className="bg-white p-8 rounded-lg shadow-sm max-w-md w-full">
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Menú no disponible</h2>
          <p className="text-gray-600">Este menú se encuentra temporalmente inactivo.</p>
        </div>
      </div>
    );
  }
  ...
  const dynamicStyles = {
    categoryTitle: { color: restaurant?.primary_color || '#1f2937' },
    categoryBorder: { backgroundColor: restaurant?.primary_color || '#d1d5db' },
  };
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-gray-800">
      <header className="bg-white/75 backdrop-blur-lg shadow-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 py-5 text-center">
          {restaurant.logo_url && (
            <img src={restaurant.logo_url} alt={`${restaurant.name} logo`} className="h-20 w-auto mx-auto mb-4 object-contain" />
          )}
      ...
      {categories.length > 1 && (
        <div className="bg-white/75 backdrop-blur-lg border-b border-gray-200 overflow-x-auto no-scrollbar sticky top-[calc(88px+2rem)] z-10">
          <div className="max-w-4xl mx-auto px-6 py-3 flex justify-center space-x-2 sm:space-x-4">
            {categories.map(cat => (
              <a key={`nav-${cat.id}`} href={`#category-${cat.id}`} className="whitespace-nowrap px-4 py-2 text-gray-600 rounded-full text-sm font-medium hover:bg-gray-100 transition-colors duration-200">
                {cat.name}
              </a>
            ))}
          </div>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 mt-8 mb-16">
        {categories.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="mt-1 text-sm text-gray-500">Este restaurante aún no ha añadido productos a su menú.</p>
          </div>
        ) : (
            ...
            {categories.map(category => (
              <section key={category.id} id={`category-${category.id}`} className="scroll-mt-40">
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-light tracking-wide" style={dynamicStyles.categoryTitle}>
                    {category.name}
                  </h2>
                  <div className="mt-2 h-0.5 w-16 mx-auto" style={dynamicStyles.categoryBorder}></div>
                </div>
                        ...
                        <div className="flex items-center gap-4 p-2">
                           {product.image_url && (
                            <div className="flex-shrink-0 bg-gray-50 rounded-lg">
                              <img src={product.image_url} alt={product.name} className="w-22 h-22 object-contain" />
                            </div>
                          )}
